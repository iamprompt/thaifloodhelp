...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Generated search embedding with', queryEmbedding.length, 'dimensions');

    // Use moderate similarity threshold (0.5) for better quality results
    const { data: similarReports, error: searchError } = await supabase.rpc('find_similar_reports', {
    ...
    console.log('Semantic search found', similarReports?.length || 0, 'similar reports');
